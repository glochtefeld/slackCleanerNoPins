...
# Create list of pinned messages
def init_pin_list(channel):
    pin_set = set()
    print('Starting jank...')
    res = slack.pins.list(channel).body
    if not res['ok']:
        return
    pins = res['items']
    print('okay')
...
def clean_channel(channel_id, time_range, pin_set, user_id=None, bot=False):
    ...
    while has_more:
        ...
        for m in messages:
            ...
            if m['type'] == 'message' and m['ts'] not in pin_set:
...
def message_cleaner():
    ...
    # Returns set of pin timestamps
    pinnedMessages = init_pin_list(_channel_id)


    # Delete messages on certain channel
    clean_channel(_channel_id, time_range,pinnedMessages, _user_id, args.bot)
